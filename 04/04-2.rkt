...
(define hehe (map (lambda (line) (map (lambda (part) (regexp-match* #rx"[0-9]+" part)) line)) split-input))
...
(define (funct n tickets)
                  (cond [(= n 0) 0]
                      [else (+ n (let loop ([n n] [tickets tickets] [sum 0])
                                   (cond [(= n 0) sum]
                                         [else (let ([recursion (funct (car tickets) (cdr tickets))])
